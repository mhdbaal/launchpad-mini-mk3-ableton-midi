...
| Note (94) / Chord (95) | switch to melodic / chord pads (press active mode's button → back to session). **Shift+Chord = NATIVE chord passthrough** (firmware chord engine, real multi-note chords + 16 user chord slots) | idem |
| Custom (96) | **native SEQUENCER passthrough** — lands on the hardware sequencer's Steps layout `(7,0,0)`; the device's own Note/Chord/Custom buttons then work natively too | idem |
