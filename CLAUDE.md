...
| Note (94) | **plain = NATIVE Note mode** (passthrough) · **Shift+Note = our melodic sequencer** (again → session) | idem |
| Chord (95) | **plain = NATIVE chord engine** (real multi-note chords + 16 user chord slots) · **Shift+Chord = our chord-pad mode** | idem |
| Custom (96) | **plain = NATIVE Custom Modes** layout `(3,0,0)` (passthrough) | idem |
| Sequencer (97) | **plain = NATIVE hardware sequencer** (Steps `(7,0,0)`) · **Shift+Sequencer = our drum sequencers**: opens `_last_drum_variant`; again while in a drum variant = variant picker; again while picker up = cancel | idem |
...
**Native passthrough** (`NATIVE_PASSTHROUGH_MODE`): plain Note/Chord/Custom/Sequencer presses step aside entirely — all components disabled (grid elements released → native-mode notes flow through port 1 straight into armed tracks, the same path that leaked notes during the wrong-port incident), then Programmer mode OFF + DAW mode + layout select (note `(4,0,0)` / chord `(2,0,0)` / custom `(3,0,0)` / sequencer steps `(7,0,0)`; full layout table in `pro/sysex_ids.py`). The device's own engines run: real chords (incl. the Chord mode's 2 white columns = user chord slots), native sequencer. **Return: press Session ON THE DEVICE** — detected by polling `layout_switch.enquire_value()` every 0.8s (notifications go to the DAW port we're not bound to; the `_passthrough_armed` flag requires one non-session reading first). TO VALIDATE on hardware: does the enquiry response arrive on port 1? Watch `layout read-back:` in the log. Manual fallback: device Setup page → Live/Programmer toggle.
